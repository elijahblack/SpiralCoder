       /// <summary>
        ...
        internal class Init
        {
            ...
            internal static void Start()
            {
                ...
                // в диалоге выбирается тип инициализации массива
                // а в этом switch'е запускается соответствующий цикл инициализации
                switch (initType)
                {
                    case "index":
                        Init.Index();
            ...
            static void Index()
            {
                ...
                // самый удобный вариант для тестирования, но
                //      это не соответствует заданию - там говорится про "посимвольное шифрование"
                // разница в том, что в посимвольном один символ - один объект, а тут один объект - целая строка
                // в принципе, можно так текст по словам записать в массив и зашифровать в случайный набор слов (но зачем?)
                for (int i = 0; i < array.GetLength(0); i++)
                {
                    for (int j = 0; j < array.GetLength(1); j++)
                    {
                        array[i, j] = "[" + i + " " + j + "]";      // запись в массив для последующего шифрования
                        originalText += "[" + i + " " + j + "]";    // запись в строку, чтобы вывести "шифруемый текст"
                    }
                }
            }
            ...
            static void Simbols()
            {
                // массив символов - это эксплуатация одной странной особенности символьного класса CHAR
                char a = '0';   // присваеваем в char '0', пока всё хорошо
                ...
                for (int i = 0; i < array.GetLength(0); i++)
                {
                    for (int j = 0; j < array.GetLength(1); j++)
                    {
                        if (count % 74 == 0) a = '0';  // если счетчик кратный 74, присвоить char'у обратно '0', потому что через 74 символа начинается нечитаемая хрень
                        array[i, j] = a.ToString();     // присваеваем символ в массив
                        originalText += a.ToString();   //      и в строку

                        a++;    // Не пытайся это понять. Да, char'у прибавляется 1. Это работает по принципу изменения индекса символа (не парься, просто используй).
                                // Если a = '0', то (a + 1) = '1', почти как int. Но если a = 'A', то (a + 1) = 'B'. 
                                // в if выше проверяется кратность 74, т.к. после 0 идут все цифры, несколько символов, весь английский алфавит в обоих регистрах
                        count++;    // и увеличиваем счетчик
                                    // Почему проверяется на %74, а не ==74? Потому что для массива 100*100, счетчик дойдёт до 100000 и первый раз он "обнулит" char, а потом нет 
                                    //      (как вариант - можно обнулять счетчик в if'е)
                    }
                }
            }
            ...
            static void Text()
            {
                 ...
                 * P.S.: Я тестил массив 101*101. Висел полчаса. Но не сломался! Можно ограничить верхнюю границу допустимого значения 
                 *          (там, где проверяется на нечетности и "N < 3")
                 *          
                 * Первое правило программиста: "Пользователь всегда идиот. Пиши код исходя из этого." */
                while (str.Length < (N * N))
                    str += str;
                ...
                for (int i = 0; i < array.GetLength(0); i++)
                {
                    for (int j = 0; j < array.GetLength(1); j++)
                    {
                        array[i, j] = str.ToCharArray()[a].ToString();      // расчленяем строку на массив символов и записываем очередной элемент в массив
                        originalText += str.ToCharArray()[a].ToString();    // и в строку
